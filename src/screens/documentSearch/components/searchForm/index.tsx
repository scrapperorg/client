...
const sourceOptions = sources.map((source: string) => (
  <MenuItem key={`sursa-document-${source}`} value={source}>
    {Translations[source]}
  </MenuItem>
));
            ...
            <FormControl fullWidth>
              ...
              >
                {Object.keys(Status).map((key: string) => (
                  <MenuItem key={`stare-document-${key}`} value={key}>
                    {Status[key]}
                  </MenuItem>
                ))}
              </Select>
        ...
        <Grid container>
          <Grid item md={3}>
            <FormControl fullWidth>
              ...
              >
                {assignableResponsibles.map((responsible) => (
                  <MenuItem
                    key={`reponsabil-${responsible.id}`}
                    value={responsible.id}
                  >{`${responsible.name} ${responsible.surname}`}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </Grid>
              ...
              render={({ field }) => (
                <Autocomplete
                  ...
                  onChange={(event: any, newValue: ProjectDto | null) => {
                    projectSearchSetOptions(
                      newValue ? [newValue, ...projectSearchOptions] : projectSearchOptions,
                    );
              ...
              render={({ field }) => (
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DatePicker
                    ...
                    renderInput={(params) => (
                      <TextField {...params} fullWidth onKeyDown={onKeyDown} />
                    )}
